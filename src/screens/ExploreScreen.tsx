...
const MOCK_ITINERARIES: PublicItinerary[] = [
  {
    ...
    description: 'Um roteiro completo pela Cidade Luz, incluindo Torre Eiffel, Louvre, Versalhes e os melhores cafés parisienses.',
    destination: {
      city: 'Paris',
      country: 'França',
  ...
  },
  {
    ...
    description: 'Trekking pelas geleiras e lagos cristalinos do fim do mundo. Inclui Torres del Paine e El Calafate.',
    destination: {
      city: 'El Calafate',
      country: 'Argentina',
  ...
  },
  {
    ...
    description: 'Descubra os melhores restaurantes, mercados e experiências culinárias da capital japonesa.',
    destination: {
      city: 'Tóquio',
      country: 'Japão',
  ...
  },
  {
    ...
    description: 'Roteiro relaxante por Cancún, Playa del Carmen e Tulum com as melhores praias e cenotes.',
    destination: {
      city: 'Tulum',
      country: 'México',
  ...
  },
  {
    ...
    description: 'Aventura histórica pelo Peru, incluindo Cusco, Vale Sagrado, Águas Calientes e a cidadela inca.',
    destination: {
      city: 'Cusco',
      country: 'Peru',
      coverImage: 'https://images.unsplash.com/photo-1587595431973-160d0d94add1?w=400&h=200&fit=crop'
    },
  ...
  },
  {
    ...
    description: 'Experiência única observando os Big Five no Kruger Park, com hospedagem em lodges de luxo.',
    destination: {
      city: 'Kruger Park',
      country: 'África do Sul',
      coverImage: 'https://images.unsplash.com/photo-1516426122078-c23e76319801?w=400&h=200&fit=crop'
    },
  ...
  },
  {
    ...
    description: 'De Salvador a Jericoacoara, passando por Maceió, Maragogi, Porto de Galinhas e Natal.',
    destination: {
      city: 'Salvador',
      country: 'Brasil',
      coverImage: 'https://images.unsplash.com/photo-1483729558449-99ef09a8c325?w=400&h=200&fit=crop'
    },
  ...
  },
  {
    ...
    description: 'Círculo Dourado, Aurora Boreal, Blue Lagoon e cachoeiras impressionantes em 8 dias.',
    destination: {
      city: 'Reykjavik',
      country: 'Islândia',
      coverImage: 'https://images.unsplash.com/photo-1504829857797-ddff29c27927?w=400&h=200&fit=crop'
    },
...
export const ExploreScreen = ({ navigation }: any) => {
      ...
      if (searchQuery) {
        ...
        filteredData = filteredData.filter(item => 
          ...
          (typeof item.destination === 'string' 
            ...
            : `${item.destination.city} ${item.destination.country}`.toLowerCase().includes(query)) ||
  ...
  const loadFeaturedItineraries = useCallback(async () => {
    try {
      // ========== USANDO DADOS MOCKADOS ==========
      await new Promise(resolve => setTimeout(resolve, 500)); // Simula delay da API
      const featuredData = MOCK_ITINERARIES
  ...
  }, [loadingMore, activeTab, pagination.hasNext, pagination.page, savedPagination.hasNext, loadDiscoverItineraries, loadSavedItineraries]);
  ...
  const handleSearch = useCallback(async () => {
    ...
    try {
      ...
      if (searchQuery.trim()) {
        await analyticsService.logDestinationSearch(
          searchQuery.trim(), 
          itineraries.length
        );
      }
  ...
  const handleToggleSave = useCallback(async (id: string) => {
    try {
      ...
      const updateSaved = (items: PublicItinerary[], isSaving: boolean) =>
        items.map(item => {
          if (item._id === id) {
            return {
              ...item,
              savesCount: isSaving ? (item.savesCount || 0) + 1 : Math.max((item.savesCount || 0) - 1, 0),
            };
          }
          return item;
        });
      ...
      if (!isSaved) {
        // Adiciona aos salvos
        const itemToSave = [...itineraries, ...featuredItineraries, ...savedItineraries].find(i => i._id === id);
        if (itemToSave) {
  ...
  const renderItineraryCard = useCallback(({ item }: { item: PublicItinerary }) => {
    ...
    return (
      ...
      >
        ...
        <Image
          source={{
            uri: item.destination?.coverImage || 'https://images.unsplash.com/photo-1488646953014-85cb44e25828?w=400&h=200&fit=crop',
          }}
        ...
        <View style={styles.cardContent}>
          ...
          <Text style={[styles.cardDestination, { color: colors.textSecondary }]}>
            {item.destination ? `${item.destination.city || 'N/A'}, ${item.destination.country || 'N/A'}` : 'Destino não informado'}
          </Text>
